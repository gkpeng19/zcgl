var IMGFOLDERPATH = 'scripts/prompt/images/';//图片路径配置
...
Dialog.prototype.show = function () {
    ...
    arr.push("    <td width='13' height='32' style=\"background-image:url(" + IMGFOLDERPATH + "dialog_lt.png) !important;background-image: none;filter:progid:DXImageTransform.Microsoft.AlphaImageLoader(src='" + IMGFOLDERPATH + "dialog_lt.png', sizingMethod='crop');\"><div style=''></div></td>");
    arr.push("    <td height='32' style=\"background-image:url(" + IMGFOLDERPATH + "dialog_ct.png) !important;background-image: none;filter:progid:DXImageTransform.Microsoft.AlphaImageLoader(src='" + IMGFOLDERPATH + "dialog_ct.png', sizingMethod='crop');\"><div style=\"float:left;font-weight:bold; color:#FFFFFF; padding:9px 0 0 4px;\"><img src=\"" + IMGFOLDERPATH + "icon_dialog.gif\" align=\"absmiddle\">&nbsp;" + this.Title + "</div>");
    arr.push("      <div style=\"position: relative;cursor:pointer; float:right; margin:6px 0 0; _margin:5px 0 0;height:17px; width:28px; background-image:url(" + IMGFOLDERPATH + "dialog_closebtn.gif)\" onMouseOver=\"this.style.backgroundImage='url(" + IMGFOLDERPATH + "dialog_closebtn_over.gif)'\" onMouseOut=\"this.style.backgroundImage='url(" + IMGFOLDERPATH + "dialog_closebtn.gif)'\" drag='false' onClick=\"Dialog.getInstance('" + this.ID + "').CancelButton.onclick.apply(Dialog.getInstance('" + this.ID + "').CancelButton,[]);\"></div></td>");
    arr.push("    <td width='13' height='32' style=\"background-image:url(" + IMGFOLDERPATH + "dialog_rt.png) !important;background-image: none;filter:progid:DXImageTransform.Microsoft.AlphaImageLoader(src='" + IMGFOLDERPATH + "dialog_rt.png', sizingMethod='crop');\"><div style=\"\"></div></td>");
    arr.push("  </tr>");
    ...
    arr.push("        <tr><td valign='top'><div style='background-color:white;padding:15px 8px 8px;text-indent:1em;position:relative;width:" + this.Width + "px;height:" + this.Height + "px;'>");
    ...
    arr.push("            <div id='_DialogButtons_" + this.ID + "' style='text-align:right; border-top:#dadee5 1px solid; padding:8px 10px; background-color:#f6f6f6;'>");
    arr.push("           	<input id='_ButtonOK_" + this.ID + "'  type='button' value='确 定'>");
    arr.push("           	<input id='_ButtonCancel_" + this.ID + "'  type='button' onclick=\"Dialog.getInstance('" + this.ID + "').close();\" value='取 消' style='display:none;'>");
    arr.push("            </div></td></tr>");
    arr.push("      </table><a href='#;' onfocus='$S(\"_forTab_" + this.ID + "\").focus();'></a></td>");
    ...
    arr.push("  <tr><td width='13' height='10' style=\"background-image:url(" + IMGFOLDERPATH + "dialog_lb.png) !important;background-image: none;filter:progid:DXImageTransform.Microsoft.AlphaImageLoader(src='" + IMGFOLDERPATH + "dialog_lb.png', sizingMethod='crop');\"></td>");
    ...
    arr.push("    <td width='13' height='10' style=\"background-image:url(" + IMGFOLDERPATH + "dialog_rb.png) !important;background-image: none;filter:progid:DXImageTransform.Microsoft.AlphaImageLoader(src='" + IMGFOLDERPATH + "dialog_rb.png', sizingMethod='crop');\"></td>");
    arr.push("  </tr></table>");
    this.TopWindow = pw;

    var bgdiv = pw.$S("_DialogBGDiv");
    if (!bgdiv) {
    ...
    var div = pw.$S("_DialogDiv_" + this.ID);
    if (!div) {
    ...
    if (this.innerElementId) {
        var innerElement = $S(this.innerElementId);
        ...
        if (isIE) {
            ...
            pw.$S("_Covering_" + this.ID).parentNode.appendChild(fragment)
        } else {
            pw.$S("_Covering_" + this.ID).parentNode.appendChild(innerElement)
        }
    }
    pw.$S("_DialogDiv_" + this.ID).DialogInstance = this;
    if (this.URL)
        pw.$S("_DialogFrame_" + this.ID).DialogInstance = this;
    pw.Drag.init(pw.$S("_draghandle_" + this.ID), pw.$S("_DialogDiv_" + this.ID));//注册拖拽方法
    if (!isIE) {
        pw.$S("_DialogDiv_" + this.ID).dialogId = this.ID;
        pw.$S("_DialogDiv_" + this.ID).onDragStart = function () { $S("_Covering_" + this.dialogId).style.display = "" }
        pw.$S("_DialogDiv_" + this.ID).onDragEnd = function () { $S("_Covering_" + this.dialogId).style.display = "none" }
    }

    this.OKButton = pw.$S("_ButtonOK_" + this.ID);
    this.CancelButton = pw.$S("_ButtonCancel_" + this.ID);

    //显示标题图片
    if (this.ShowMessageRow) {
        $E.show(pw.$S("_MessageRow_" + this.ID));
        if (this.MessageTitle) {
            pw.$S("_MessageTitle_" + this.ID).innerHTML = this.MessageTitle;
        }
        if (this.Message) {
            pw.$S("_Message_" + this.ID).innerHTML = this.Message;
        }
    }

    //显示按钮栏
    if (!this.ShowButtonRow) {
        pw.$S("_ButtonRow_" + this.ID).hide();
    }
    if (this.CancelEvent) {
...
Dialog.prototype.close = function () {
    if (this.innerElementId) {
        var innerElement = $E.getTopLevelWindow().$S(this.innerElementId);
    ...
    if (this.WindowFlag) {
        ...
        if (this.AlertFlag) {
            $E.hide(pw.$S("_AlertBGDiv"));
        }
        if (!flag && !this.AlertFlag) {//此处是为处理弹出窗口被关闭后iframe立即被重定向时背景层不消失的问题
            pw.eval('window._OpacityFunc = function(){var w = $E.getTopLevelWindow();$E.hide(w.$S("_DialogBGDiv"));}');
            pw._OpacityFunc();
        }
...
Dialog.prototype.addButton = function (id, txt, func) {
    ...
    var pw = $E.getTopLevelWindow();
    pw.$S("_DialogButtons_" + this.ID).$T("input")[0].getParent("a").insertAdjacentHTML("beforeBegin", html);
    pw.$S("_Button_" + this.ID + "_" + id).onclick = func;
}
...
Dialog.alert = function (msg, func, w, h) {
    ...
    diag.show();
    pw.$S("_AlertBGDiv").style.display = "";
    $E.hide(pw.$S("_ButtonOK_" + diag.ID));
    var win = pw.$S("_DialogFrame_" + diag.ID).contentWindow;
    ...
    pw.$S("_DialogButtons_" + diag.ID).style.textAlign = "center";
}
...
Dialog.confirm = function (msg, func1, func2, w, h) {
    ...
    diag.show();
    pw.$S("_AlertBGDiv").style.dispaly = "";
    var win = pw.$S("_DialogFrame_" + diag.ID).contentWindow;
    var doc = win.document;
    doc.open();
    doc.write("<body oncontextmenu='return false;'></body>");
    ...
    if (d) {
        try {
            ...
            if (!this.AlertFlag) {
                ...
                if (!flag) {
                    $E.getTopLevelWindow().$S("_DialogBGDiv").style.opacity = "0.4";
                    $E.getTopLevelWindow().$S("_DialogBGDiv").style.filter = "alpha(opacity=40)";
                }
            }
            if (d.AlertFlag) {
                $E.show($E.getTopLevelWindow().$S("_AlertBGDiv"));
            }
...
Dialog.setPosition = function () {
    ...
    for (i = 0; i < DialogArr.length; i++) {
        pw.$S("_DialogDiv_" + DialogArr[i]).DialogInstance.setPosition();
    }
}
Dialog.prototype.setPosition = function () {
    ...
    //pw.$S(this.bgdivID).style.width= sw + "px";
    pw.$S(this.bgdivID).style.height = sh + "px";
}
