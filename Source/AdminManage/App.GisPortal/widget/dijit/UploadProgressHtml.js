...
function (declare, lang, topic, parser, _WidgetBase, arrayUtils, SWFUpload, _TemplatedMixin, on, dom, domConstruct, domStyle, Measurement, Units, template) {
    ...
    clazz = declare([_WidgetBase, _TemplatedMixin], {
        ...
        createProgressNode: function (file) {
            var fileSize = this.getFileSize(file.file.size);
            var htmlStr = "<div class='jz-progress-item'><div class='jz-progress-col1' style='width:44%'>" + file.file.name + "</div><div class='jz-progress-col1' style='width:17%'>" + fileSize + "</div><div id='file_item_" + file.index + "' class='jz-progress-col1' style='width:13%'></div><div id='file_oper_" + file.index + "' class='jz-progress-col1' style='color:#F00'>上传中...</div><div id='file_progress_" + file.index + "' class='progressor'></div></div>";
            domConstruct.place(htmlStr, this.containerNode, "last");
        },
        setProgress: function (index, percent) {
            dom.byId("file_progress_" + index).style.width = percent + '%';
            dom.byId("file_item_" + index).innerHTML = percent + '%';
            if (percent >= 100) {
                dom.byId("file_oper_" + index).innerHTML = '上传完成';
            }
        },
        setError: function (index) {
            dom.byId("file_oper_" + index).innerHTML = '上传失败';
        },
        close: function () {
